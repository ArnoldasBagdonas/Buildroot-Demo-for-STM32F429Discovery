...
int pwm_open(pwm_t *pwm, unsigned int chip, unsigned int channel)
{
    ...
    if (stat(channel_path, &stat_buf) < 0)
    {
        ...
        /* Wait until the exported PWM channel directory appears. */
        unsigned int retry_count;
        for (retry_count = 0; retry_count < PWM_EXPORT_STAT_RETRIES; retry_count++)
        {
            if ((ret = stat(channel_path, &stat_buf)) < 0 && errno != ENOENT)
        ...
        /* Loop until period is writable. The sysfs attributes can appear a
         * short time after export, even on systems without udev. */
        for (retry_count = 0; retry_count < PWM_EXPORT_STAT_RETRIES; retry_count++)
        {
            if ((fd = open(path, O_WRONLY)) < 0)
            {
        ...
        if (retry_count == PWM_EXPORT_STAT_RETRIES)
            return _pwm_error(pwm, PWM_ERROR_OPEN, 0, "Opening PWM: waiting for 'pwm%u/period' timed out", channel);
    }
