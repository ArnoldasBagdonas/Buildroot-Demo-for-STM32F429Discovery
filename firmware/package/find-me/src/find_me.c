...
#define DEFAULT_STATE_FILE "/var/lib/find-me/state"
...
_Static_assert(sizeof(PersistentState) <= NVMEM_STATE_MAX_PAYLOAD,
               "find-me state does not fit one NVMEM record");
#endif
#ifdef FMD_SPINOR_STATE
_Static_assert(sizeof(PersistentState) <= SPINOR_STATE_MAX_PAYLOAD,
               "find-me state does not fit one SPI-NOR record");
#endif
...
static void controls_init(Application *application)
{
    ...
    if (controls->button_fd < 0)
        fprintf(stderr, "find_me_button_unavailable chip=%s line=%u error=%s\n",
                application->options.button_chip, application->options.button_line,
                strerror(errno));
    controls->led_fd =
        request_gpio_line(application->options.led_chip, application->options.led_line,
                          GPIO_V2_LINE_FLAG_OUTPUT, "find-me-led");
    if (controls->led_fd < 0)
        fprintf(stderr, "find_me_led_unavailable chip=%s line=%u error=%s\n",
...
static void debug_network_status(Application *application, uint64_t now)
{
    ...
    printf("find_me_debug_net carrier=%u tx_packets=%llu tx_errors=%llu "
...
static void set_confirmation_pattern(Application *application, unsigned int flashes)
{
    ...
    printf("find_me_confirmation_pattern=%s\n",
...
static void confirm_physical(Application *application)
{
    ...
    if (application->oauth.physical_confirmed)
    {
        ...
        fputs("find_me_physical_confirmation=accepted\n", stdout);
        fflush(stdout);
    }
}
...
{
    PlatformControls *controls = &application->controls;
    printf("find_me_debug_led_state=%s gpio=%s link=%s\n",
...
static void reload_interface_address(Application *application)
{
    ...
    if (descriptor < 0)
    {
        perror("find-me mDNS address reload");
        return;
    }
    ...
    if (inet_ntop(AF_INET, &address, printable, sizeof(printable)) != NULL)
        printf("find_me_ipv4=%s source=dhcp-update\n", printable);
    fflush(stdout);
}
...
static int application_run(Application *application)
{
    ...
    while (!stop_requested)
    {
        ...
        if (confirm_requested)
        {
            confirm_requested = 0;
#ifdef FMD_CONSOLE_DEBUG
            fputs("find_me_debug_button=tap\n", stdout);
            fflush(stdout);
#endif
            confirm_physical(application);
        }
...
int main(int argc, char **argv)
{
    ...
    if (application.options.prepare_interface)
    {
        ...
        printf("find_me_mac=%02x:%02x:%02x:%02x:%02x:%02x\n", mac[0], mac[1], mac[2], mac[3],
               mac[4], mac[5]);
        return 0;
    }
