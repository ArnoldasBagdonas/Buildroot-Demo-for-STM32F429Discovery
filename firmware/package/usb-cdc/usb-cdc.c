...
int main(int argc, char **argv)
{
	...
	if (fd < 0) {
		fprintf(stderr, "usb-cdc: cannot open %s: %s\n",
			device, strerror(errno));
		return EXIT_FAILURE;
	}

	if (tcgetattr(fd, &saved_settings) < 0) {
		fprintf(stderr, "usb-cdc: tcgetattr(%s): %s\n",
			device, strerror(errno));
		close(fd);
		return EXIT_FAILURE;
	}
	...
	if (tcsetattr(fd, TCSANOW, &raw_settings) < 0) {
		fprintf(stderr, "usb-cdc: tcsetattr(%s): %s\n",
			device, strerror(errno));
		close(fd);
		return EXIT_FAILURE;
	}
	...
	while (!stop_requested) {
		...
		if (received < 0) {
			if (errno == EINTR)
				continue;
			fprintf(stderr, "usb-cdc: read(%s): %s\n",
				device, strerror(errno));
			result = EXIT_FAILURE;
			break;
		}
		if (received == 0) {
			fprintf(stderr, "usb-cdc: host disconnected\n");
			break;
		}

		if (write_all(fd, buffer, (size_t)received) < 0) {
			fprintf(stderr, "usb-cdc: write(%s): %s\n",
				device, strerror(errno));
			result = EXIT_FAILURE;
			break;
		}
	...
	printf("usb-cdc: stopped\n");
	fflush(stdout);

	if (tcsetattr(fd, TCSANOW, &saved_settings) < 0) {
		fprintf(stderr, "usb-cdc: cannot restore %s settings: %s\n",
			device, strerror(errno));
		result = EXIT_FAILURE;
	}
	close(fd);
