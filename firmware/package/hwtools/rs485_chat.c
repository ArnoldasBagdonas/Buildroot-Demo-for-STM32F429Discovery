...
static void rs485_cleanup(int exit_code)
{
    if (rs485_fd >= 0)
        close(rs485_fd);
...
static void rs485_print_help(const char *progname)
{
    printf("\nUsage: %s <tty device>\n", progname);
...
static int configure_serial(const char *tty_path)
{
    struct termios tty;
...
static int configure_rs485(void)
{
