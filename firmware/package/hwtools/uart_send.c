...
static void uart_cleanup(int exit_code)
{
    if (serial)
    {
        serial_close(serial);
...
static void uart_print_help(const char *progname)
{
    printf("\nUsage: %s <tty device>\n", progname);
...
int uart_send_main(int argc, char *argv[])
{
    ...
    uart_cleanup(EXIT_SUCCESS); // This will never be reached but good practice
}
